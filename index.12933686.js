var JsonView = function(exports) {
    ...
    function createNodeElement(node1) {
        var el = document.createElement('div');
        ...
        if (node1.children.length > 0) {
            el.innerHTML = expandedTemplate({
                key: node1.key,
                size: getSizeString(node1)
            });
            var caretEl = el.querySelector('.caret-icon');
            caretEl.addEventListener('click', function() {
                toggleNode(node1);
            });
        } else el.innerHTML = notExpandedTemplate({
            key: node1.key,
            value: node1.value,
            type: _typeof(node1.value)
        });
        var lineEl = el.children[0];
        if (node1.parent !== null) lineEl.classList.add('hide');
        lineEl.style = 'margin-left: ' + node1.depth * 18 + 'px;';
        return lineEl;
    }
    ...
    function traverseTree(node, callback) {
        callback(node);
        if (node.children.length > 0) node.children.forEach(function(child) {
    ...
    function createSubnode(data, node) {
        if (_typeof(data) === 'object') for(var key in data){
